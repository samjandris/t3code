...
export type TerminalMetadataStreamEvent =
  | { readonly type: "snapshot"; readonly terminals: ReadonlyArray<TerminalSummary> }
...
export type TerminalAttachInput = Omit<TerminalOpenInput, "terminalId"> & {
  readonly terminalId: string;
...
export type TerminalAttachStreamEvent =
  ...
  | TerminalEvent
  | { readonly type: "closed"; readonly threadId: string; readonly terminalId: string };
...
export interface TerminalAttachClient {
  readonly terminal: {
    readonly open: (input: TerminalAttachInput) => Promise<TerminalSessionSnapshot>;
    readonly onEvent: (
      listener: (event: TerminalEvent) => void,
      options?: { readonly onResubscribe?: () => void },
    ) => () => void;
...
export function createTerminalSessionManager(config: TerminalSessionManagerConfig) {
    ...
    switch (event.type) {
      ...
      case "exited":
        ...
        applyMetadataEvent(target, {
          type: "upsert",
          terminal: {
            ...(getMetadata(knownTarget.environmentId)[keyFromKnownTarget(knownTarget)]?.summary ??
              summaryFromSnapshot({
      ...
      case "activity":
        {
          const existing = getMetadata(knownTarget.environmentId)[keyFromKnownTarget(knownTarget)]
            ?.summary;
          if (existing) {
            applyMetadataEvent(target, {
              type: "upsert",
  ...
  function attach(input: {
    ...
    let active = true;
    const unsubscribe = input.client.terminal.onEvent((event) => {
      if (
        event.threadId !== input.terminal.threadId ||
        event.terminalId !== input.terminal.terminalId
      ) {
        return;
      }
    ...
    void input.client.terminal
      .open(input.terminal)
      .then((snapshot) => {
        if (!active) return;
        const event: TerminalAttachStreamEvent = { type: "snapshot", snapshot };
        applyAttachEvent({ environmentId: input.environmentId }, event);
        input.onEvent?.(event);
      ...
      .catch((error: unknown) => {
        ...
        const event: TerminalAttachStreamEvent = {
          ...
          terminalId: input.terminal.terminalId ?? "default",
