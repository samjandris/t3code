...
function toDerivedWorkLogEntry(activity: OrchestrationThreadActivity): DerivedWorkLogEntry {
  ...
  const toolSummary =
    !isTaskActivity && toolSummaryStatus === "complete"
      ? extractPayloadToolDetail(payload, title ?? activity.summary)
      : null;
  const detail = isTaskActivity
    ...
    : toolSummaryStatus === "complete"
      ? extractToolDataDetail(payload, title ?? activity.summary)
...
function extractPayloadToolDetail(
  payload: Record<string, unknown> | null,
  heading: string,
): string | null {
...
function extractToolDataDetail(
  payload: Record<string, unknown> | null,
  heading: string,
): string | null {
  const normalizedHeading = normalizePreviewForComparison(heading);
...
function extractToolDetail(
  ...
  return extractPayloadToolDetail(payload, heading) ?? extractToolDataDetail(payload, heading);
}
