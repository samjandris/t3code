...
export function MobileWorkspaceSheet(props: {
  ...
  const presentation = resolveMobileSheetPresentation(Platform.OS);
  ...
  return (
    ...
    >
      <KeyboardAvoidingView
        className={isIos ? "flex-1" : "flex-1 justify-end"}
        ...
        >
          ...
          {props.workspaceMode === "worktree" ? (
            <Pressable
              ...
              style={{
                borderColor: props.startFromOrigin ? String(primaryColor) : String(borderColor),
                backgroundColor: props.startFromOrigin
                  ? "rgba(59,130,246,0.12)"
                  : String(subtleColor),
              }}
              accessibilityRole="switch"
              accessibilityState={{ checked: props.startFromOrigin }}
              onPress={() => props.onStartFromOriginChange(!props.startFromOrigin)}
            >
              <View className="min-w-0 flex-1 pr-3">
                <Text className="text-[14px] font-t3-bold text-foreground">Start from origin</Text>
                <Text className="text-[12px] text-foreground-muted">
                  Base the worktree on the latest origin branch
                </Text>
              </View>
              <SymbolView
                name={props.startFromOrigin ? "checkmark.circle.fill" : "circle"}
                size={20}
                tintColor={String(props.startFromOrigin ? primaryColor : iconSubtleColor)}
                type="monochrome"
              />
            </Pressable>
          ) : null}
