...
function dataWithToolCallId(event: ProviderRuntimeEvent): Record<string, unknown> | undefined {
  ...
  return {
    ...base,
    toolCallId:
      typeof base?.toolCallId === "string" && base.toolCallId.length > 0
        ? base.toolCallId
        : event.itemId,
  };
}
...
function toolActivityId(
  ...
  return EventId.make(`tool:${event.threadId}:${event.turnId ?? "thread"}:${event.itemId}`);
}
      ...
      return [
        {
          ...
          payload: {
            ...
            ...(options.taskTitle ? { title: truncateDetail(options.taskTitle, 120) } : {}),
